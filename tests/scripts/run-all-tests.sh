...
print_header "xpushare GPU 共享测试套件"

# 检查 xpushare 组件状态
echo "检查 xpushare 组件状态..."
if ! kubectl get pods -n xpushare-system 2>/dev/null | grep -q "Running"; then
    echo -e "${RED}✗${NC} 错误：xpushare 组件未正常运行"
    echo "请先部署 xpushare 组件："
    echo "  kubectl apply -f .tests/manifests/"
    exit 1
fi
echo -e "${GREEN}✓${NC} xpushare 组件正常运行"
echo ""
...
echo "4. 显存边界测试（启用 XPUSHARE_ENABLE_SINGLE_OVERSUB）"

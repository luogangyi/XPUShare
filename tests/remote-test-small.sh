...
if [ "$SKIP_SETUP" == "true" ]; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
        ...
        git add .
        git commit -m "wip: auto-commit by remote-test-small.sh [$(date +%H:%M:%S)]"
    else
    ...
    # Also clean up other known apps to free resources?
...
echo "===== 6. Running Test (Small Workload) ====="
# Run with default 10 pods or passed arg
"$SCRIPT_DIR/scripts/test-small.sh" "$@"

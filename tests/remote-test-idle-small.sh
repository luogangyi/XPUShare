...
if [ "$SKIP_SETUP" == "true" ]; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
        ...
        git add .
        git commit -m "wip: auto-commit by remote-test-idle-small.sh [$(date +%H:%M:%S)]"
    else
    ...
    echo "===== 2. Remote Build ====="
    # We need to build the specific idle image
    ssh $SSH_OPTS "$REMOTE_USER@$REMOTE_HOST" "cd $REMOTE_DIR && make all && cd tests && make build-pytorch-idle-small push-pytorch-idle-small"
...
# Run with default 10 pods or passed arg
"$SCRIPT_DIR/scripts/test-idle-small.sh" "$@"

...
#define log_fatal_errno(fmt, ...)                                  \
  do {                                                             \
    fprintf(stderr, "[XPUSHARE][FATAL]: " fmt "\n", ##__VA_ARGS__); \
...
#define log_info(fmt, ...)                                        \
  do {                                                            \
    fprintf(stderr, "[XPUSHARE][INFO]: " fmt "\n", ##__VA_ARGS__); \
  } while (0)

#define log_warn(fmt, ...)                                        \
  do {                                                            \
    fprintf(stderr, "[XPUSHARE][WARN]: " fmt "\n", ##__VA_ARGS__); \
  } while (0)
...
#define log_debug(fmt, ...)                                          \
  do {                                                               \
    if (__debug)                                                     \
      fprintf(stderr, "[XPUSHARE][DEBUG]: " fmt "\n", ##__VA_ARGS__); \
  } while (0)

#define true_or_exit(condition)                                      \
  do {                                                               \
    if (!(condition)) log_fatal("Condition failed: %s", #condition); \
  } while (0)
...
#define XPUSHARE_UNREGISTERED_ID 0xF00DF00DF00DF00D

#define ENV_XPUSHARE_DEBUG "XPUSHARE_DEBUG"

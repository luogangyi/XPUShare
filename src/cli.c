...
xoptOption options[] = {
    {"set-tq", 'T', offsetof(SimpleConfig, cmdline_scheduler_tq), 0,
     XOPT_TYPE_INT, "n",
     "Set the time quantum of the scheduler to TQ seconds. Only"
     " accepts positive integers."},
    {"anti-thrash", 'S', offsetof(SimpleConfig, cmdline_anti_thrash), 0,
     XOPT_TYPE_STRING, "s",
     "Set the desired status of the scheduler. Only accepts values"
     " \"on\" or \"off\"."},
    {"help", 'h', offsetof(SimpleConfig, help), 0, XOPT_TYPE_BOOL, 0,
...
int main(int argc, const char* argv[]) {
  ...
  if (config.cmdline_anti_thrash != NULL) {
    if (strcmp(config.cmdline_anti_thrash, "on") == 0)
      ...
      log_fatal(
          "Invalid option for --anti-thrash (-S). Must"
          " be one of 'on' or 'off'.");
  ...
  if (config.cmdline_scheduler_tq != 0) {
    ...
    if (parsed_scheduler_tq <= 0)
      log_fatal(
          "Invalid option for --set-tq. TQ value"

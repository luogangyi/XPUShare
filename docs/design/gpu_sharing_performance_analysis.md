...
本文档针对用户在测试 `xpushare` 时提出的两个关键疑问进行深度技术分析，特别是针对性能严重下降 (95%) 的根本原因。
...
**根本原因在于 `xpushare` 的动态流控机制 (Pending Kernel Window) 陷入了"死亡螺旋"。**

### 机制分析
`xpushare` 为了防止某个进程提交过多 Kernel 导致 GPU 锁无法释放，引入了一个流控窗口 (`Pending Kernel Window`)：
...
因此，我们坚决推荐：
**配置 `XPUSHARE_SWITCH_TIME_MODE=fixed` 且 `VALUE=600` (10分钟)**，让大显存任务尽可能一口气跑完，避免触发流控降级。

...
本文档详细描述了如何验证 `xpushare` 在多 GPU 环境下的功能正确性，以及显存超额订阅 (Oversubscription) 能力的测试方法。
...
*   已安装 NVIDIA Driver 和 Kubernetes Device Plugin (NVIDIA 官方插件需禁用或与 xpushare 共存配置正确)。
*   `xpushare` 组件 (Scheduler, Device Plugin) 已成功部署。

### 1.2 部署检查
确保 `xpushare` 节点已上报虚拟资源：
```bash
kubectl describe node <gpu-node> | grep xpushare.com/gpu
# 预期输出：xpushare.com/gpu: 20 (假设 2 张卡，每张虚拟化 10 个)
```
...
1.  **创建测试 Pod 定义 (test-pods.yaml)**
    创建两个 Pod，申请 `xpushare.com/gpu: 1`。
...
### 3.1 目标
验证两个通过 `xpushare` 共享不同 GPU 的 Pod，其调度是独立的，互不阻塞。

### 3.2 测试方法
1.  在 `gpu-pod-1` (绑定 GPU A) 运行持续计算任务。
2.  在 `gpu-pod-2` (绑定 GPU B) 运行持续计算任务。
3.  观察宿主机的 `nvidia-smi` 或 `xpushare-scheduler` 日志。

### 3.3 预期结果
*   宿主机上看到两张 GPU 的利用率都上升。
*   `xpushare-scheduler` 日志显示两个不同的 Context 都在进行 `LOCK_OK` / `DROP_LOCK` 的循环（如果开启了时间片抢占），或者长期持有锁。
...
int main(int argc, char **argv) {
    ...
    char *d_ptr;
    // 注意：这里代码写 cudaMalloc，但运行时会被 libxpushare 劫持为 cudaMallocManaged
...
1.  部署 Pod，**不设置** `XPUSHARE_ENABLE_SINGLE_OVERSUB`。
...
3.  查看 `xpushare-scheduler` 日志。

### 5.3 预期结果
*   日志显示 `LOCK_OK` 依次发送给 Client 1 -> 2 -> 3 -> 4 -> 5 -> 1...
*   每个 Client 持有锁的时间大约等于配置的 `XPUSHARE_DEFAULT_TQ` (默认 30ms-50ms)。
